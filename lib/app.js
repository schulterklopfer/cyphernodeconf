...
module.exports = class App {
  ...
  async setupConfigArchive() {
    ...
    if( versionOverride ) {
      ...
      defaults.gatekeeper_version =
        defaults.proxy_version =
          defaults.proxycron_version =
            defaults.pycoin_version =
              defaults.otsclient_version =
                defaults.bitcoin_version =
                  defaults.lightning_version = 'latest';
      defaults.sparkwallet_version = 'standalone'
  ...
  async writeFiles() {
    ...
    console.log( chalk.green( '   create' )+' config.7z' );
    ...
    console.log( chalk.green( '   create' )+' client.7z' );
  ...
  getDefault(name ) {
    if( !this.config || !this.config.data || !this.config.data[name] ) {
      return;
    }
