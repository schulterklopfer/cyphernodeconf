...
const defaultConfigFileVersion='0.1.0';
...
module.exports = class Config {
  ...
  constructor() {
    ...
    this.setData( { __version: latestConfigFileVersion } );
  ...
  setData( data ) {
    ...
    this.data = data;
    this.data.__version = this.data.__version || defaultConfigFileVersion;
    this.validate();
  }
  ...
  async deserialize( path, password ) {
    ...
    if( this.data.__version !== latestConfigFileVersion ) {
      ...
      fs.copyFileSync( path, path+'-'+this.data.__version );
      await this.migrate(this.data.__version, latestConfigFileVersion );
  ...
  async migrate_0_1_0_to_0_2_0() {
    if( this.data.__version != '0.1.0' ) {
      return;
    }

    this.data.__version = '0.2.0';
    ...
    for( let i=0; i<gatekeeper_keys.configEntries.length; i++ ) {
      ...
      apiKey.setFomConfigEntry(gatekeeper_keys.configEntries[i]);
